...
async function showViewer(number){
  ...
  app.innerHTML = `
    ...
    <button id="previousCut" class="side-arrow left" type="button" aria-label="이전 컷">‹</button>
    <button id="nextCut" class="side-arrow right" type="button" aria-label="다음 컷">›</button>
  `;
  ...
  function render(){
    ...
    if(mode==="card"){
      ...
      previousCutButton.classList.remove("hidden");
      nextCutButton.classList.remove("hidden");
      ...
      previousCutButton.classList.add("hidden");
      nextCutButton.classList.add("hidden");
